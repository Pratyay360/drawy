...
export function CanvasEditor({ id }: CanvasEditorProps) {
	...
	useEffect(() => {
		...
		window.addEventListener("canvas-updated", refreshCanvas);
		const unsubscribe = subscribeCanvasEvents(refreshCanvas);
		return () => {
			window.removeEventListener("canvas-updated", refreshCanvas);
			unsubscribe();
		};
	}, [id, fetchCanvas]);

	// Live, cross-user collaboration: broadcast scene changes, react to other
	// editors' updates, and track who else is viewing this canvas.
	useEffect(() => {
