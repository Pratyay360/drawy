...
export function useCanvasLifecycle({ id }: UseCanvasLifecycleOptions) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        setModuleError(null);
        const mod = await withTimeout(import("@excalidraw/excalidraw"), 15000, "Loading editor");
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      ...
      try {
        ...
        await withTimeout(saveCanvas(id, elements, appState, prunedFiles), 12000, "Saving canvas");
        setSaveStatus("saved");
