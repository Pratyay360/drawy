...
function getItemName(item: LibraryItem | undefined, index: number, itemNames: string[]): string {
...
export function LibraryItemBrowser({ library, onBack, onRefreshContent }: LibraryItemBrowserProps) {
  ...
  const items = useMemo(() => {
    ...
    return raw
      ...
      .filter(
        ({ name, searchText }) =>
          !lowerQuery || name.toLowerCase().includes(lowerQuery) || searchText.includes(lowerQuery),
      );
        ...
        <Center>
          <VStack gap={3} hAlign="center">
            ...
            <VStack gap={1} hAlign="center">
              ...
              <Text type="supporting">Download this library to browse and use its items.</Text>
            </VStack>
            <Button
