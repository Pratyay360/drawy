...
export function ShareCanvasModal() {
	...
	const setShareIsTogglingPublic = useUIStore(
		(s) => s.setShareIsTogglingPublic,
	);
	...
	async function handleRemoveShare(usernameToRemove: string) {
		...
		try {
			...
			throw new Error(
				`Failed to unshare canvas: ${error instanceof Error ? error.message : String(error)}`,
			);
		} finally {
								...
								{isPublic && (
									<HStack gap={2} align="center">
										...
										<Button
											...
											size="sm"
											icon={
												<Icon icon={publicCopied ? Check : Copy} size="sm" />
											}
