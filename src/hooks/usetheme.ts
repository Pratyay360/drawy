...
function setStateInternal(next: Partial<ThemeState>) {
	...
	if (
		...
		merged.modePreference === state.modePreference
	) {
		return;
	}
	state = merged;
	...
	applyDocumentState(
		state.name,
		getEffectiveMode(state.name, state.modePreference),
	);
...
export function useTheme() {
	...
	useEffect(() => {
		applyDocumentState(
			snapshot.name,
			getEffectiveMode(snapshot.name, snapshot.modePreference),
		);
